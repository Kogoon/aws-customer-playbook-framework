...
## 이 저장소는 무엇인가요?
 이 파일 모음은 AWS 서비스를 사용할 때의 잠재적인 공격 시나리오에 대비하여 보안 플레이북을 만들고 개발하고 통합하기 위한 예시 프레임워크로 제공되고 있습니다. [한국어의 플레이북은 이쪽](./docs/kr)

 이 플레이북에는 [AWS Translate](https://aws.amazon.com/translate/)에 의한 기계 번역을 사용하여 이루어진 번역이 포함되어 있습니다. 해당 플레이북은 사람이 번역에 대한 검토를 하지 않았기 때문에 오류가 있을 수 있습니다. 사용 전, 반드시 문서를 확인하여 개별 사용 사례에 적합한 문맥과 언어를 확인하세요. 오류 수정이나 오류 확인에 도움을 주신다면 언제든 환영합니다. 
